...
//        mSizeForTimeLine = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP,13, APlayerApplication.getContext().getResources().getDisplayMetrics());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mLyrics.size(); i++) {
            if(i == mCurRow){   //画高亮歌词
                drawLyric(canvas,mPaintForHighLightLrc,availableWidth,mLyrics.get(i));
            }else{  //普通歌词
                //计算歌词透明度
//                int alpha = (int)(0xff /  Math.pow(Math.abs(i - mCurRow),1.2f));
//                mPaintForOtherLrc.setAlpha(alpha);
                drawLyric(canvas,mPaintForOtherLrc, availableWidth, mLyrics.get(i));
            }
        }
        //画时间线和时间
        if(mIsDrawTimeLine){
            float y = getHeight() / 2 + getScrollY();
            canvas.drawText(mLyrics.get(mCurRow).getTimeStr(), 0, y - 5, mPaintForTimeLine);
     ...
     * @param Lyric
     */
    private void drawLyric(Canvas canvas, TextPaint textPaint, int availableWidth, Lyric Lyric) {
        drawText(canvas,textPaint,availableWidth,Lyric.getContent());
        mRowY += Lyric.getContentHeight();
        if(Lyric.hasTranslate()){
            drawText(canvas,textPaint,availableWidth,Lyric.getTranslate());
            mRowY += Lyric.getTranslateHeight();
        }
//        mRowY += Lyric.getHeight();
        mRowY += mLinePadding;
    }
            ...
            case MotionEvent.ACTION_MOVE:
                if(hasLrc()){
                    ...
                    if(mCanDrag){
                        ...
                        }else if(getScrollY() - offset > mLyrics.size() * (mSizeForOtherLrc + mLinePadding) - mLinePadding){
                            if(offset < 0 ){
                                offset = offset / 3;
                            }
                        }
...
//                        int currentRow = (int) (getScrollY() / (mSizeForOtherLrc + mLinePadding));
                        ...
                        currentRow = Math.min(currentRow, mLyrics.size() - 1);
                        currentRow = Math.max(currentRow, 0);
                        seekTo(mLyrics.get(currentRow).getTime(), false,false);
                        return true;
                    }
                    mLastY = event.getRawY();
                } else {
            ...
            case MotionEvent.ACTION_UP:
                if(!mCanDrag){
                    ...
                    if(onSeekToListener!= null && mCurRow != -1){
                        onSeekToListener.onSeekTo(mLyrics.get(mCurRow).getTime());
                    }
                    if(getScrollY() < 0){
    ...
    public void setLyrics(List<Lyric> Lyrics) {
        ...
        if(mLyrics != null){
            //计算每一行歌词所占的高度
//            for(Lyric Lyric : mLyrics){
//                int height = 0;
//                String combine = Lyric.getContent() + (!TextUtils.isEmpty(Lyric.getTranslate()) ? "\t" + Lyric.getTranslate() : "");
...
//                }
//                Lyric.setHeight(height);
//            }
            for(Lyric Lyric : mLyrics){
                Lyric.setContentHeight(getSingleLineHeight(Lyric.getContent()));
                if(Lyric.hasTranslate()){
                    Lyric.setTranslateHeight(getSingleLineHeight(Lyric.getTranslate()));
                }
                Lyric.setTotalHeight(Lyric.getTranslateHeight() + Lyric.getContentHeight());
            }
        }
        invalidate();
    }
    ...
    private int getScrollYByRow(int row){
        int scrollY = 0;
        for(int i = 0 ;i < mLyrics.size() &&  i < row;i++){
            scrollY += mLyrics.get(i).getTotalHeight() + mLinePadding;
        }
        return scrollY;
    }
    ...
    private int getRowByScrollY(){
        int totalY = 0;
        int line;
        for(line = 0; line < mLyrics.size();line++){
            totalY += mLinePadding + mLyrics.get(line).getTotalHeight();
    ...
    public void seekTo(int progress,boolean fromSeekBar,boolean fromSeekBarByUser) {
        ...
        for (int i = mLyrics.size() - 1; i >= 0; i--) {
            if(progress >= mLyrics.get(i).getTime()){
                if(mCurRow != i){
                    mLastRow = mCurRow;
                    mCurRow = i;
...
//					float textWidth = mPaintForHighLightLrc.measureText(mLyrics.get(mCurRow).getContent());
//					log("textWidth="+textWidth+"getWidth()=" + getWidth());
//					if(textWidth > getWidth()){
//						if(fromSeekBarByUser){
//							mScroller.forceFinished(true);
//						}
//						log("开始水平滚动歌词:" + mLyrics.get(mCurRow).getContent());
//						startScrollLrc(getWidth() - textWidth, (long) (mLyrics.get(mCurRow).getTotalTime() * 0.6));
//					}
                    invalidate();
                }
                break;
